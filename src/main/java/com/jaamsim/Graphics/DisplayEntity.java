...
public class DisplayEntity extends Entity {
	...
	@Output(name = "DependentEntityList",
	 description = "The entities whose inputs include one or more references to this entity.",
	    sequence = 12)
	public ArrayList<DisplayEntity> getDependentEntityList(double simTime) {
		ArrayList<DisplayEntity> ret = new ArrayList<>();
		for (DisplayEntity ent : getJaamSimModel().getClonesOfIterator(DisplayEntity.class)) {
			if (ent == this || ent instanceof OverlayEntity || ent instanceof Region)
				continue;
