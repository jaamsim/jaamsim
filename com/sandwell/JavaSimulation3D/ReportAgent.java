...
public class ReportAgent extends DisplayEntity {
	...
	protected String executableName;  // Name of the executable file, or null if it does not exist
	...
	protected Vector lastReportIntervalTime; // time of the last report printing
	...
	public ReportAgent() {
		...
		lastReportIntervalTime = Clock.calculateYearMonthDayHourForTime( simulation.getInitializationTime() );
		groupReportFileName = "";
	...
	public void printReportHeaderOn( FileEntity anOut ) {
		...
		anOut.format( "Run Description:  %s\n", simulation.getRunDescription() );
	...
	public void printGroupReportOn( FileEntity anOut ) {
		...
		Vector startTime = lastReportIntervalTime;
		Vector endTime = Clock.calculateYearMonthDayHourForTime( getCurrentTime() );

		anOut.newLine();
		anOut.putString( "Group Report ( Period From "+startTime.get( 0 )+" / "+startTime.get( 1 )+" / "+startTime.get( 2 )+" : "+startTime.get( 3 )+" To "+endTime.get( 0 )+" / "+endTime.get( 1 )+" / "+endTime.get( 2 )+" : "+endTime.get( 3 )+"  )" );
		anOut.newLine();
		anOut.putString( "==================" );
		anOut.newLine( 2 );
		...
		for (Group each : groupList) {
			this.printGroupUtilizationOn(each, anOut);
			anOut.newLine();
		}
 	}
	 ...
	 */
	public void printGroupUtilizationOn(Group grp, FileEntity anOut) {
		// If there are no elements in the group, do nothing
		if (grp.getList().size() == 0)
			return;

		// Print the name of the group
		anOut.putString(grp.getName());
		anOut.newLine();
		for (int i = 0; i < grp.getName().length(); i++) {
			anOut.putString("-");
		}
		anOut.newLine();

		// Print the header and determine if the values on the bottom line are TOTAL, AVERAGE, or BLANK
		ModelEntity ent = ((ModelEntity)grp.getList().get(0));
		IntegerVector bottomLine = ent.printUtilizationHeaderOn( anOut );

		// Set up the totals, minimum, and maximum for the columns
		DoubleVector columnValues;
		...
		for (int i = 0; i < grp.getList().size(); i++) {
			ent = ((ModelEntity)grp.getList().get(i));
			if( ent.isActive() ) {

				// Print and store the values for this member
				columnValues = ent.printUtilizationOn( anOut );
